...
public interface MensajeRepositorio extends JpaRepository<Mensaje, String> {
       ...
       @Query("SELECT m FROM Mensaje m WHERE m.pedido.id = :idPedido AND m.rol <> (SELECT u.rol FROM Usuario u WHERE u.id = :idUsuario)")
       public List<Mensaje> msjsRecibidosPorPedido( @Param("idPedido") String idPedido);
