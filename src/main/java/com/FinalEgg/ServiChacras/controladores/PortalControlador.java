...
public class PortalControlador {
   ...
   @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_MIXTO')")
   @GetMapping("/inivarios")
...
public String registro(@RequestParam String nombre, @RequestParam String apellido, @RequestParam String email,
                       @RequestParam String password, @RequestParam String password2, @RequestParam(required = false) Barrio barrio,
                       @RequestParam(required = false) String rol, @RequestParam String direccion, @RequestParam String telefono,
                       @RequestParam(required = false) MultipartFile archivo, @RequestParam(required = false) String descripcion,
                       @RequestParam(required = false) String idServicio, ModelMap modelo, HttpSession session) {
    ...
    if (posibleUsuario != null) {
        ...
        modelo.put("detalle", "El correo ya se encuentra registrado");
    ...
    try {
        ...
        if (rol.equalsIgnoreCase("proveedor") || rol.equalsIgnoreCase("mixto")) {
            ...
            if (descripcion == null || descripcion.equals("") || idServicio == null) {
                usuarioRepositorio.deleteById(usuario.getId());

                String detalleError = "Los siguientes datos no pueden estar nulos: ";
                if (idServicio == null) {
                    detalleError += "servicio";
                }
                if (descripcion == null || descripcion.equals("")) {
                    if (idServicio == null) {
                        detalleError += ", ";
                    }
                    detalleError += "descripción";
                }

                modelo.put("error", detalleError);
        ...
        modelo.put("error", "El usuario no pudo ser registrado");
        modelo.put("detalle", nombre + " " + apellido + "\n" + email + ": " + email);
