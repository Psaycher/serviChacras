...
public class NotificacionControlador {
    ...
    public String verNotificacion(@PathVariable String id, HttpSession session, ModelMap modelo) throws MiExcepcion {
        ...
        if (opcionalNotificacion.isPresent()) {
            ...
            boolean autoMiradaBoton = false;
            String textoAutoMirada = "";

            if (String.valueOf(notificacion.getNota()).equals("PEDIDOASolicitud")) { 
                Optional<Pedido> opcionalPedido = Optional.ofNullable(pedidoRepositorio.getPorDetalle(notificacion.getDetalle()));
                boolean aceptado = false;

                if (opcionalPedido.isPresent()) {
                    aceptado = true;
                    autoMiradaBoton = true;
                    textoAutoMirada = "Pedido aceptado"; 

                } else { textoAutoMirada = "Pedido rechazado"; }

                modelo.addAttribute("aceptado", aceptado);                 
            }
            
            if (String.valueOf(notificacion.getNota()).equals("PEDIDOAceptado")) { 
                Pedido pedido = pedidoServicio.getOne(notificacion.getPedido());
                boolean pedidoAlta = pedido.isAlta();              
                
                if (!pedidoAlta) {
                    autoMiradaBoton = true;
                    textoAutoMirada = "Pedido cancelado";  
                }
                hayPedido = "HAY";                     
            }
            ...
            if (String.valueOf(notificacion.getNota()).equals("PEDIDOFinalizado")) { 
                Optional <List<Notificacion>> opcionalNotificaciones = Optional.ofNullable(notificacionRepositorio.listarPorPedido(notificacion.getPedido()));
                
                if (opcionalNotificaciones.isPresent()) {
                    List<Notificacion> listaNotificaciones = opcionalNotificaciones.get();

                    for (Notificacion noti : listaNotificaciones) {
                        if (noti.getCorrespondencia().equals(id)) {
                            autoMiradaBoton = true;
                            textoAutoMirada = "Pedido finalizado"; 
                        }
                    }  
                }
                hayPedido = "HAY";     
            ...
            if (String.valueOf(notificacion.getNota()).equals("CONFIRMARFinalizacion")) { 
                if (!notificacion.getPedido().isEmpty()) {
                    autoMiradaBoton = true;
                    textoAutoMirada = "Finalización confirmada"; 
                }  
                hayPedido = "HAY";   
            }

            if (String.valueOf(notificacion.getNota()).equals("PAGODemandado")) {                
                if (!notificacion.getPago().isEmpty()) {
                    autoMiradaBoton = true;
                    textoAutoMirada = "Pago Demandado"; 
                } 
                hayPago = "HAY";
            }

            if (String.valueOf(notificacion.getNota()).equals("PAGOEfectuado")) { 
                if (!notificacion.getPago().isEmpty()) {
                    autoMiradaBoton = true;
                    textoAutoMirada = "Pago efectuado"; 
                } 
                hayPago = "HAY"; 
            }

            if (String.valueOf(notificacion.getNota()).equals("DENUNCIA")) { 
                if (!notificacion.getDenuncia().isEmpty()) {
                    autoMiradaBoton = true;
                    textoAutoMirada = "Denuncia efectuada"; 
                } 
                hayDenuncia = "HAY"; 
            }

            if (String.valueOf(notificacion.getNota()).equals("DENUNCIARechazada")) { 
                hayDenuncia = "HAY"; 

                if (!notificacion.getDenuncia().isEmpty()) { 
                    autoMiradaBoton = true;
                    textoAutoMirada = "Denuncia Rechazada"; 
                } 
                hayDenuncia = "HAY"; 
            }
            ...
            modelo.addAttribute("autoMiradaBoton", autoMiradaBoton);
            modelo.addAttribute("textoAutoMirada", textoAutoMirada);
